...
for k = 1:length(files)
    ...
    try
        ...
        if ~isempty(time(varQC <= 2))
            p(pn) = plot(time(varQC <= 2), var(varQC <= 2),'.-','MarkerSize',5,'LineWidth',0.9,'DisplayName',num2str(nominal_depth), 'Color', cl(k,:));
        else
            p(pn) = plot(time, var,'.','MarkerSize',5,'LineWidth',0.9,'DisplayName',num2str(nominal_depth), 'Color', cl(k,:));
        end
        

        % plot the QC info
        hold on
        Q2 = plot(time(varQC == 2), var(varQC == 2), 'o', 'MarkerSize',6, 'MarkerFaceColor', cl(k,:), 'Color', cl(k, :),'DisplayName',num2str(nominal_depth), 'Color', cl(k,:));

        Q3 = plot(time(varQC == 3), var(varQC == 3),'d','MarkerSize',6,'MarkerFaceColor', cl(k,:), 'Color', cl(k, :),'DisplayName',num2str(nominal_depth), 'Color', cl(k,:));

        Q4 = plot(time(varQC == 4), var(varQC == 4),'s','MarkerSize', 6, 'MarkerFaceColor', cl(k,:), 'Color', cl(k, :),'DisplayName',num2str(nominal_depth), 'Color', cl(k,:));
...
% set(get(get(Q3,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
% set(get(get(Q4,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');

[a,bb,icons,c]= legend(p,'Orientation','horizontal','Location', 'southoutside','FontSize', 12);
icons(1).MarkerSize = 12;
icons(2).MarkerSize = 12;
icons(3).MarkerSize = 12;
