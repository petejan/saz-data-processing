...
netcdf.putAtt(ncid, glob_att, 'deployment_code', deployment{1});
...
netcdf.putAtt(ncid, glob_att, 'geospatial_lat_max', str2double(deployment_data.cmdddlatitude{dep_info_idx}));
netcdf.putAtt(ncid, glob_att, 'geospatial_lat_min', str2double(deployment_data.cmdddlatitude{dep_info_idx}));
netcdf.putAtt(ncid, glob_att, 'geospatial_lat_units', 'degrees_north');
netcdf.putAtt(ncid, glob_att, 'geospatial_lon_max', str2double(deployment_data.cmdddlongitude{dep_info_idx}));
netcdf.putAtt(ncid, glob_att, 'geospatial_lon_min', str2double(deployment_data.cmdddlongitude{dep_info_idx}));
...
netcdf.putAtt(ncid, glob_att, 'instrument_serial_number', '10705-01');
...
netcdf.putAtt(ncid, glob_att, 'time_deployment_start', datestr(deployment_data.cmddddeploymentdate(dep_info_idx), time_fmt));
netcdf.putAtt(ncid, glob_att, 'time_deployment_end', datestr(deployment_data.cmdddrecoverydate(dep_info_idx), time_fmt));

netcdf.putAtt(ncid, glob_att, 'title', 'SAZ Mooring Data');
...
% loop through variables, and create them in netCDF, creating the
% quality_control ones also
next_qc = 0;        
var_ids = [];
var_n = 1;
for var_name = data.Properties.VariableNames(13:end) % TODO: find the start of variable names, 13 is a bit of a hack
    disp(var_name)
...
% latitude, longitude
netcdf.putVar(ncid, lat_id, str2double(deployment_data.cmdddlatitude{dep_info_idx}));
netcdf.putVar(ncid, lon_id, str2double(deployment_data.cmdddlongitude{dep_info_idx}));

% data variables
next_qc = 0;     
var_n = 1;   
for var_name = data.Properties.VariableNames(13:end)
    disp(var_name)
    vn = var_name{1};
    ...
    if iscell(var_data_raw)
        ...
        var_data_raw = cell2mat(cellfun(@str2num, var_data_raw, 'UniformOutput', false));
    end
